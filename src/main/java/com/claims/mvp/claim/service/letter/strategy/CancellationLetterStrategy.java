...
/**
 * Letter for flight cancellation claims under EU 261/2004.
 */
@Component
public class CancellationLetterStrategy implements LetterStrategy{
    ...
    public LetterResponse generateLetter(Claim claim) {
        ...
        String subject = "EU 261 Compensation Claim - " + flight.getFlightNumber()
                + " (" + flight.getRouteFrom() + " -> " + flight.getRouteTo() + ")";

        Integer notceDays = issue.getCancellationNoticeDays();
        String incidentLine = "My flight was cancelled " + (notceDays == null ? "without notice" : notceDays + " days before the flight date.");
