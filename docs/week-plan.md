...
**Цель.** Сейчас при переходе в `SUBMITTED` уведомление уходит только пользователю. Претензия в авиакомпанию не отправляется — 
                           ключевой gap Phase 1. Это самый важный шаг: без него продукт генерирует письмо но никуда не отправляет.
